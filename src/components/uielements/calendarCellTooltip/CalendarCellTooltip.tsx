...
const TooltipMetric = ({ title, value }: { title: string; value: string | number }) => (
...
const TooltipPriceMetric = ({ title, value }: { title: string; value: string | number }) => (
  <div className="text-center">
    <div className="text-xs text-muted-secondary font-medium uppercase tracking-wide mb-1">{title}</div>
...
const RiskLevelBadge = ({ cell }: { cell: HeatmapCell }) => {
  ...
  return (
    <div className="text-center">
      <div className="text-xs text-muted-secondary font-medium uppercase tracking-wide mb-1">Risk Level</div>
      <div className={cn('text-sm font-bold px-3 py-1 rounded-full inline-block border', riskInfo.classes)}>
        {riskInfo.text}
      </div>
    </div>
  );
        ...
        {cell ? (
          <>
            <div className="grid grid-cols-2 gap-4">
              <TooltipMetric title="Vol (Daily)" value={`${cell.volatilityDaily.toFixed(2)}%`} />
              <TooltipMetric
                title="Vol (Rolling)"
                value={cell.volatilityRolling !== null ? `${cell.volatilityRolling.toFixed(2)}%` : '—'}
              />
              <TooltipMetric title="Liquidity" value={formatCompact(cell.liquidity)} />
              <TooltipMetric
                title="Performance"
            ...
            <div className="grid grid-cols-2 gap-4">
              <TooltipPriceMetric title="High" value={cell.high?.toFixed(2) || '—'} />
              <TooltipPriceMetric title="Low" value={cell.low?.toFixed(2) || '—'} />
            </div>
